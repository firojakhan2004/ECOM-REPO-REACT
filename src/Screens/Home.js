...
function Home() {
  ...
  function getMyCart() {
    axios
      .get(Base_URL + "get-my-cart", { params: { u_id: u_id } })
      .then((result) => {
  ...
  const addToCart = (el) => {
    ...
    axios
      .post(Base_URL + "add-to-cart", data)
      .then((res) => {
          ...
          <div class="card-body">
            ...
            <p class="card-text">
              {el.description.length > 20 ? (
                <>
                  <sapn>{el.description.slice(0, 20) + "... "}</sapn>{" "}
                  <a
                    style={{ fontSize: 12 }}
                    href=""
                    onClick={() => handleViewMore(el)}
            ...
            <p class="card-title" style={{ fontSize: 25, fontWeight: "bold" }}>
              {" "}
              {el.discount > 0 ? (
                <span
            ...
            <a onClick={() => handleViewMore(el)} class="btn btn-primary">
              View More
            </a>
            {el.disable == true ? (
              <button
